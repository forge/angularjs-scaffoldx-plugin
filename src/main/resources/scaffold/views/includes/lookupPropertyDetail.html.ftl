...
<#if (property.hidden!"false") == "false">
    <div class="control-group" ng-class="{error: ${formProperty}.$invalid}">
        ...
        <select id="${property.name}" name="${property.name}" ng-model="${modelProperty}" ng-options="${collectionVar} for ${collectionVar} in ${collection}" <#if (property.required!) == "true">required</#if> >
