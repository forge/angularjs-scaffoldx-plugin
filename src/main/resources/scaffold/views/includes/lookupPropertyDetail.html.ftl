...
<#if (property.hidden!"false") == "false">
    <div class="control-group" ng-class="{error: ${formProperty}.$invalid}">
        ...
        <div class="controls">
        <select id="${property.name}" name="${property.name}" ng-model="${modelProperty}" ng-options="${property.name?substring(0, 1)} as ${property.name?substring(0, 1)} for ${property.name?substring(0, 1)} in ${property.name}List" <#if (property.required!) == "true">required</#if> >
